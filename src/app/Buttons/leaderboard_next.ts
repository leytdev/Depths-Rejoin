  ...
  async run(interaction: ButtonInteraction) {
    ...
    try {
      ...
      if (leaderboard.length === 0) {
        await interaction.followUp({
          content: 'На следующей странице нет данных.',
          ephemeral: true
        });
        return;
      }
...
Сообщений: **${userData.dailyMessageCount || 0}** | Онлайн: **${voiceString}**`;
      ...
      const newEmbed = new EmbedBuilder()
        ...
        .setColor('#2f3136')
        .setFooter({ text: `Depths • Страница ${nextPage} • Ваш ранг: #${userRank}` })
        .setTimestamp()
      ...
      ErrorHandler.logError(`Leaderboard Next Button Handler`, error);
      await interaction.followUp({
